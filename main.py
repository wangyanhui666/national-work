...
def save(gdp, product1, product2, product3, income, outcome, name):
    ...
    # 创建表data
    try:
        c.execute('create table data(gdp real, product1 real, product2 real, product3 real,income real, outcome real)')
        conn.commit()
    except:
        pass
    # 判断表是否为空
    pastdata = c.execute("SELECT gdp, product3 FROM data;")
    counter = 0
...
def judgeblank(name):
    ...
    c = conn.cursor()
    try:
        c.execute('create table data(gdp real, product1 real)')
        conn.commit()
        print()
    except:
        pass
    # 判断表是否为空
    pastdata = c.execute("SELECT gdp, product1 FROM data;")
    counter = 0
    for item in pastdata:
        counter += 1
        if counter >= 1:
...
if __name__ == '__main__':
    ...
    judge = judgeblank("work.db")
    if judge == 0:
        # 发出请求下载数据和存到本地数据库
        data.download()
        data.handle_gdp()
        data.handle_come()
        save(data.GDP, data.product1, data.product2, data.product3, data.income, data.outcome, "work.db")
    # 从数据库中调取数据
    GDP = []
    Product1 = []
    Product2 = []
    ...
    Outcome = []
    # 调取
    outtable(GDP, Product1, Product2, Product3, Income, Outcome, "work.db")
    # 画gdp的图
    years_product = np.arange(1999, 2019, 1)
    # 第一个figure
    f = plot.figure(figsize=(20, 20))
    # 第一个子图
    plot.subplot(2, 1, 1)
    plot.plot(years_product, GDP, color='red', marker='o', linestyle='solid', label='GDP')
